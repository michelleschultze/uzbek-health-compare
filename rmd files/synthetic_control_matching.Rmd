...
match.out.min <- c("doctors_per_capita", "IMR_all", "LOS", "pc_beds", 
                   ...
                   "pc_genitourinary", "pc_injury", "pc_TB", "pc_hosp", "pop")
...
sea1 <- comparisons %>%
  ...
  drop_na() %>%
  microsynth(idvar="Region", timevar="year", intvar="Intervention",
             start.pre=2002, end.pre=NULL, end.post=2018,
             match.out.min=match.out.min,
             match.out = FALSE,
             match.covar.min = FALSE,
             match.covar = FALSE,
             result.var=match.out.min, omnibus.var=match.out.min,
             test="lower",
#             perm=250, jack = TRUE, check.feas=TRUE, use.backup = TRUE,
