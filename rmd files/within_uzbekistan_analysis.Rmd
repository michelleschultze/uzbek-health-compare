...
```{r doc-plots}
ggplot(uz_data, aes(x = year, y = doctors_per_capita, color = Region)) +
  geom_line() +
...
## TBA: Add national level analysis? (goes for all)
```
...
ggplot(nat_uz_data, aes(x = year, y = doctors_per_capita)) +
  geom_line(size=1.25) +
...
We have evidence to support that doctors per capita have decreased by approximately 0.24 doctors per 10000 population each year in Uzbekistan. However, we do not have evidence to support a linear relationship between doctors per capita and year during this time period in Tashkent City.
...
```{r fert-plots}
ggplot(uz_data, aes(x = year, y = fert_rate, color = Region)) +
  geom_line() +
...
ggplot(nat_uz_data, aes(x = year, y = fert_rate)) +
  geom_line(size=1.25) +
  geom_line(data = filter(uz_data, Region == "Tashkent city" | Region == "Navoi" | Region == "Jizzakh" | Region == "Syrdarya" | Region == "Tashkent"), aes(x = year, y = fert_rate, color = Region)) +
  labs(title = "Fertility Rate", subtitle = "Uzbekistan national level", x = "Year", y = "Fertility Rate")
```
